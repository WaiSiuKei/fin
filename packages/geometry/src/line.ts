import { EPSILON, isZero } from '@fin/numerical';
import { ILine, IVector } from './def';
...
export class Line implements ILine {
  ...
  constructor(p: IVector, v: IVector) {
    this._px = p.x;
    this._py = p.y;
    this._vx = v.x;
    this._vy = v.y;
  ...
  static fromTwoPoint(p1: IVector, p2: IVector): ILine {
    return {
      px: p1.x,
      py: p1.y,
      vx: p2.x - p1.x,
  ...
  static getSide(line: ILine, v: IVector, isInfinite = false): number {
  ...
  static signedDistanceBetween(line: ILine, v: IVector): number {
    const { px, py, vx, vy } = line;
    const { x, y } = v;
  ...
  static distanceBetween(line: ILine, v: IVector): number {
