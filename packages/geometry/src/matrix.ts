import { IMatrix, IMatrixDecomposition, IVector } from './def';
import { Vector } from './vector';
import { isArray } from '@fin/types';
...
export class Matrix {
  ...
  constructor()
  constructor(mx: DOMMatrix)
  constructor(mx: IMatrix)
  constructor(mx: any = Matrix.initial) {
    if (mx instanceof DOMMatrix) {
  ...
  static translate(mx: IMatrix, point: IVector): IMatrix {
    const { x, y } = point;
    let tx = mx.tx + x * mx.a + y * mx.c;
    let ty = mx.ty + x * mx.b + y * mx.d;
    return { a: mx.a, b: mx.b, c: mx.c, d: mx.d, tx, ty };
  }
  static scale(mx: IMatrix, scale: IVector, center?: IVector): IMatrix {
  ...
  static rotate(mx: IMatrix, angle: number, center: IVector = { x: 1, y: 1 }): IMatrix {
    angle *= Math.PI / 180;
  ...
  static shear(mx: IMatrix, shear: IVector, center?: IVector): IMatrix {
  ...
  static skew(mx: IMatrix, skew: IVector, center?: IVector) {
    const toRadians = Math.PI / 180;
    const shear = { x: Math.tan(skew.x * toRadians), y: Math.tan(skew.y * toRadians) };
    return Matrix.shear(mx, shear, center);
  }
  static append(mx1: IMatrix, mx2: IMatrix): IMatrix {
  ...
  static prepend(mx1: IMatrix, mx2: IMatrix): IMatrix {
  ...
  static invert(mx: IMatrix): IMatrix {
    const { a, b, c, d, tx, ty } = mx;
    let det = a * d - b * c;
    let res = null;
    if (det && !isNaN(det) && isFinite(tx) && isFinite(ty)) {
  ...
  static isIdentity(mx: IMatrix): boolean {
    return mx.a === 1 && mx.b === 0 && mx.c === 0 && mx.d === 1 && mx.tx === 0 && mx.ty === 0;
  }
  static isInvertible(mx: IMatrix): boolean {
    let det = mx.a * mx.d - mx.c * mx.b;
    return det && !isNaN(det) && isFinite(mx.tx) && isFinite(mx.ty);
  }
  static isSingular(mx: IMatrix): boolean {
    return !Matrix.isInvertible(mx);
  }
  static decompose(mx: IMatrix): IMatrixDecomposition {
    // http://dev.w3.org/csswg/css3-2d-transforms/#matrix-decomposition
    // http://www.maths-informatique-jeux.com/blog/frederic/?post/2013/12/01/Decomposition-of-2D-transform-matrices
    // https://github.com/wisec/DOMinator/blob/master/layout/style/nsStyleAnimation.cpp#L946
  ...
  static inverseTransform(mx: IMatrix, point: IVector): IVector {
  ...
  static toArray(mx: IMatrix): [number, number, number, number, number, number] {
    return [mx.a, mx.b, mx.c, mx.d, mx.tx, mx.ty];
  }
  static parse(str: string | [number, number, number, number, number, number]) {
    let match;
    let f = parseFloat;
    if (isArray(str)) {
      return new Matrix({
