...
import { INPUT_FONT_DEFAULTS } from './editorOptions';
...
export class BareFontInfo {
  ...
  public static createFromRawSettings(opts: {
    ...
    let fontFamily = _string(opts.fontFamily, INPUT_FONT_DEFAULTS.fontFamily);
    let fontWeight = _string(opts.fontWeight, INPUT_FONT_DEFAULTS.fontWeight);

    let fontSize = safeParseFloat(opts.fontSize, INPUT_FONT_DEFAULTS.fontSize);
    fontSize = clamp(fontSize, 0, MAXIMUM_FONT_SIZE);
    if (fontSize === 0) {
      fontSize = INPUT_FONT_DEFAULTS.fontSize;
