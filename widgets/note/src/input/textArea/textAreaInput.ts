...
import { IKeyboardEvent, KeyCode } from '@fin/keyboard';
import { ITextAreaWrapper, ITypeData, TextAreaState } from './textAreaState';
import { addDisposableListener, FastDomNode, addStandardDisposableListener, restoreParentsScrollTop, saveParentsScrollTop } from '@fin/dom';
import { isEdgeOrIE, isMacintosh, isChrome, isIE, isWindows, isEdge, isFirefox } from '@fin/platform';
...
export class TextAreaInput extends Disposable {
  ...
  constructor(host: ITextAreaInputHost, textArea: FastDomNode<HTMLTextAreaElement>) {
    ...
    this._register(addStandardDisposableListener(textArea.domNode, 'keydown', (e: IKeyboardEvent) => {
    ...
    this._register(addStandardDisposableListener(textArea.domNode, 'keyup', (e: IKeyboardEvent) => {
      this._onKeyUp.fire(e);
    }));

    this._register(addDisposableListener(textArea.domNode, 'compositionstart', (e: CompositionEvent) => {
    ...
    this._register(addDisposableListener(textArea.domNode, 'compositionupdate', (e: CompositionEvent) => {
    ...
    this._register(addDisposableListener(textArea.domNode, 'compositionend', (e: CompositionEvent) => {
    ...
    this._register(addDisposableListener(textArea.domNode, 'input', () => {
      ...
      const [newState, typeInput] = deduceInputFromTextAreaValue(/*couldBeEmojiInput*/isMacintosh, /*couldBeTypingAtOffset0*/previousEventWasFocus && isMacintosh);
    ...
    this._register(addDisposableListener(textArea.domNode, 'cut', (e: ClipboardEvent) => {
    ...
    this._register(addDisposableListener(textArea.domNode, 'copy', (e: ClipboardEvent) => {
      this._lastTextAreaEvent = TextAreaInputEventType.copy;

      this._ensureClipboardGetsEditorSelection(e);
    }));

    this._register(addDisposableListener(textArea.domNode, 'paste', (e: ClipboardEvent) => {
  ...
  private _ensureClipboardGetsEditorSelection(e: ClipboardEvent): void {
    ...
    if (!ClipboardEventUtils.canUseTextData(e)) {
      // Looks like an old  The strategy is to place the text
