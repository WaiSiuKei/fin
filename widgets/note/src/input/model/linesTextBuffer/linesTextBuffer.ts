...
import { ApplyEditsResult, IIdentifiedSingleEditOperation, IInternalModelContentChange, ISingleEditOperationIdentifier, ITextBuffer } from '../model';
import { EndOfLinePreference } from '../../model';
import { firstNonWhitespaceIndex, containsRTL, isBasicASCII, lastNonWhitespaceIndex } from '@fin/strings';
import { PrefixSumComputer } from '../../viewModel/prefixSumComputer';
import { ModelRawChange, ModelRawLineChanged, ModelRawLinesDeleted, ModelRawLinesInserted } from '../textModelEvents';
import { arrayInsert } from '@fin/arrays';
...
export class LinesTextBuffer implements ITextBuffer {
  ...
  public getLineFirstNonWhitespaceColumn(lineNumber: number): number {
    const result = firstNonWhitespaceIndex(this._lines[lineNumber - 1]);
    if (result === -1) {
      return 0;
    }
    return result + 1;
  ...
  public getLineLastNonWhitespaceColumn(lineNumber: number): number {
    const result = lastNonWhitespaceIndex(this._lines[lineNumber - 1]);
    if (result === -1) {
      return 0;
    }
    return result + 2;
  ...
  private _doApplyEdits(operations: IValidatedEditOperation[]): [ModelRawChange[], IInternalModelContentChange[]] {
    ...
    for (let i = 0, len = operations.length; i < len; i++) {
      ...
      if (editingLinesCnt < insertingLinesCnt) {
        ...
        this._lines = arrayInsert(this._lines, startLineNumber + editingLinesCnt, newLines);
